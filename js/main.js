...
// V0.1: filtro “Casa”
let hideCasa = false; // false = se muestran “compra casa/garaje” y “venta casa”

/* ==========================
   UTILIDADES (hash & PIN en utils.js)
========================== */
// sha256, esc, getAttempts, setAttempts, getCooldownUntil, setCooldown, isInCooldown
// están definidas en utils.js y ya se cargan antes de main.js
...
function mostrar() {
  ...
  // === Gestión dinámica de botones en footer ===
  ...
  btnLeft.onclick = null; btnCenter.onclick = null;

  const modo = movDiv.dataset.modo || "lista";

  if (modo === "graficos") {
    // izquierda: volver a lista
...
const renderizarBarrasGraficos = (f) => {
  ...
  // aplicar filtro Casa si procede
  ...
  lista.innerHTML += html + Object.entries(totales)
    .sort((a,b)=>b[1]-a[1])
    .map(([cat,val])=>{
      ...
      return `<div class="card" style="border:none;background:transparent">
        ...
        <div class="bar-flex" style="width:${(val/max)*100}%">
          ${esCasa
            ? `<div class="bar-segment" style="width:100%;background:var(--success)"></div>`
...
function renderizarGraficos2() {
  ...
  // filtros activos: usamos Ori/Cat/Sub; ignoramos Mes/Año para mostrar 13 meses siempre
  const fs = ["filtroMes","filtroAño","filtroCat","filtroSub","filtroOri"].map(id => document.getElementById(id).value);
  ...
  // base filtrada por otros filtros
  const filtraOtros = (m) => {
    const cC = fs[2] === "TODAS" || m.c === fs[2];
    const cS = fs[3] === "TODAS" || m.s === fs[3];
    const cO = fs[4] === "TODOS" || m.o === fs[4];
...
const abrirGraficos = () => {
  const m = document.getElementById("movimientos");
  // Alternar entre lista y gráficos
...
window.onscroll = () => {
  const m = document.getElementById("movimientos");
  if (m.dataset.modo === "graficos" || m.dataset.modo === "graficos2") return;
