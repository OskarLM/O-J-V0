...
// Devuelve el nombre de mes (es) a partir de una fecha ISO (yyyy-MM-dd)
function mesFromISO(iso) {
  try {
    ...
    return mesesLabel[new Date().getMonth()];
  }
}
...
function renderizarGraficos2() {
  ...
  const filtraOtros = (m) => {
    ...
    const cS = fs[3] === "TODAS" || m.s === fs[3];
    const cO = fs[4] === "TODOS" || m.o === fs[4];
    // FIX: devolvemos conjunción, no el último valor
    return cC && cS && cO;
  };
...
// 👉 Mueve combos según Origen (arregla Nómina: Oskar/Josune + Mes de la fecha)
function onOrigenChange(origenValor, { preCat = "", preSub = "" } = {}) {
  ...
  if (origenValor === "Nómina") {
    ...
    NOMINA_CATS.forEach(c => {
      selCat.innerHTML += `<option value="${c}" ${c === preCat ? 'selected' : ''}>${c}</option>`;
    });

    // --- Subcategoría: meses (preferimos el mes de la fecha del formulario)
    ...
    NOMINA_SUBS.forEach(m => {
      selSub.innerHTML += `<option value="${m}" ${m === mesPrefer ? 'selected' : ''}>${m}</option>`;
    });
  } else {
...
const abrirFormulario = (id = null) => {
  ...
  if (id) {
    ...
    // Rellena combos según origen (Nómina ⇒ Oskar/Josune + mes de la fecha)
...
// Inicia login con scopes de archivos
